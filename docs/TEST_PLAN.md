...
| UI-03 | Reintento rápido | 1. Repetir UI-01 pero con `fastMode` ON. | Se obtiene respuesta y se lanza auditoría en background (spinner/hint). |
| UI-04 | Flujo sin IA real | 1. Remover `GOOGLE_API_KEY`. 2. Repetir UI-01. | Resolución muestra mensaje “Modo demo” con datos simulados, sin alertas rojas. |

## 3. Pruebas API

- **API-CHAT-200**: `curl -X POST /api/chat` con payload válido y `GOOGLE_API_KEY` configurado. Verificar `resolution` y `sources`.
- **API-CHAT-DEM0**: Igual que anterior pero sin `GOOGLE_API_KEY`. Debe devolver contenido simulado y `hallazgo` “Modo demo”.
